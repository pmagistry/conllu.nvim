# -*- coding:utf-8 -*-
# original author: hankcs for hanlp
from typing import Union, List
import sys


from visualization import pretty_tree_horizontal, make_table


class CoNLLWord:
    def __init__(self, id, form, lemma=None, upos=None, pos=None, feats=None, head=None, deprel=None, phead=None,
                 pdeprel=None):
    ...
    def __str__(self):
        if isinstance(self.head, list):
            return '\n'.join('\t'.join(['_' if v is None else v for v in values]) for values in [
                [str(self.id), self.form, self.lemma, self.upos, self.pos, self.feats,
                 None if head is None else str(head), deprel, self.phead, self.pdeprel] for head, deprel in
                zip(self.head, self.deprel)
            ])
        values = [str(self.id), self.form, self.lemma, self.upos, self.pos, self.feats,
    ...
    def nonempty_fields(self):
        ...
        return list(f for f in
                    [self.form, self.lemma, self.upos, self.pos, self.feats, self.head, self.deprel, self.phead,
                     self.pdeprel] if f)
    ...
    def get_pos(self):
        ...
        Returns: ``self.pos`` or ``self.upos``.

        """
        return self.pos or self.upos
...
class CoNLLSentence(list):
    ...
    def to_tree(self, extras: List[str] = None) -> str:
        ...
        has_upos = all(x.upos for x in self)
        if has_lem:
            rows[0].append('Lemma')
        if has_pos:
            rows[0].append('PoS')
        ...
        for i, (word, arc) in enumerate(zip(self, tree)):
            ...
            if has_upos:
                cell_per_word.append(word.upos)
            if extras:
...
if __name__ == "__main__":
    conllu_txt = sys.stdin.read()
    sent = CoNLLSentence.from_str(conllu_txt)
    print(sent.to_tree())
